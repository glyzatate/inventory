...
<h3>AVR Details</h3>
<?php 			
		$where = "avr_id = ".$_GET['id'];				
		$details = $db->selectSingleQueryArray("avr","*", $where,"");				

		?>				
<table border="0">
	<?php echo '
		<tr bgcolor="#E0E0E0">
			<td width="20%">AVR Label</td>		
			<td>:<td>
			<td >'.$details["avr_label"].'</td>			
		</tr>
		<tr>
			<td >AVR Brand</td>		
			<td>:<td>
			<td >'.$details["avr_brand"].'</td>			
		</tr>
		<tr bgcolor="#E0E0E0">
			<td >AVR Model</td>		
			<td>:<td>
			<td >'.$details["avr_model"].'</td>			
		</tr>
		<tr>
			<td >AVR Serial</td>		
			<td>:<td>
			<td >'.$details["avr_serial"].'</td>			
		</tr>
	</table>'; ?>	
<hr>
...
<h3>AVR Assignment History</h3>
	<table border="1" >
		...
		<?php 
		$addON = "LEFT JOIN staff ON  uid = assign_userid LEFT  JOIN avr ON  avr_id = assign_avrid";
		$where = "assign_avrid = ".$_GET['id'];
